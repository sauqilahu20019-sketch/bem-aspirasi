...
class Mahasiswa extends Component
{
    ...
    public function store()
    {
        $validated = $this->validate([
            ...
            'prodi.string' => 'Prodi harus berupa Text!',
            'prodi.max' => 'Max. Prodi 255 Karakter',
    ...
    {
        if ($this->search) {
            $users = User::onlyMahasiswa()->where('full_name', 'like', '%' . $this->search . '%')
                ->orWhere('nim', 'like', '%' . $this->search . '%')
                ->latest()->paginate($this->perPage);
        } else {
            $users = User::onlyMahasiswa()->paginate($this->perPage);
        }
